 ...
 * there is no match, filling in missing reads from a given list and only 
 * writing the higher quality of data if there are index collisions.
...
static const size_t BUFFER_SIZE = 60000;
...
void insertQuery(char *next_query, char *read, int first_index, int last_index, int max_index, int quality);
...
double findQuality(char *header);
  
double qualities[60000];
char *read = NULL;
...
{
	if (argc < 3) {
                printf("Usage: ./concatQual <input filename>* <List of reads>\n");
                exit(1);
        }
...
// initiate necesary vars, read first line of input file and write header
	...
	quality = findQuality(header_line);
...
// Create the char array to be written to be written to the new file
	...
	for (int i = 0; i < max_index; i++) qualities[i] = 0;	
	fgets(first_query, abs(query_length) * 2, file);

	// copy the contents of the query to the read, possibly inverted
	insertQuery(first_query, read, first_index, last_index, max_index, quality);
	
///////////////////////////////////////////////////////////////////////////////
// continue while the file has more content
	while (NULL != fgets(next_line, BUFFER_SIZE, file))
	{
		...
		insertQuery(next_query, read, first_index, last_index, max_index, quality);
...
char* findHeader(char* line) 
{
	...
	// read scaffold
	i = findChar(i, line, '.'); // move pointer to after the first scaffold 
	i = findChar(i, line, ' ');
	...
	// parse the quality of the query
	i = findChar(i, line, 'N') + 4;
	num_digits = findChar(i, line, ';') - i;
	for (int j = 0; j < num_digits; j++)
		quality[j] = *(line + i + j);
	...
	return_string[1 + strlen(read_name) + strlen(head)] = 33; // '!'

	// concatenate the quality data
	for (int j = 0; j < strlen(quality); j++)
		return_string[j + 2 + strlen(read_name) + strlen(head)] = quality[j];
	return_string[2 + strlen(read_name) + strlen(head) + strlen(quality)] = 0;
...
void insertQuery(char *next_query, char *read, int first_index, int last_index, int max_index, int quality)
{
	...
	if (last_index > first_index) {
			...
			if (j + first_index - 1 < max_index + 2) { 
				if (qualities[j + first_index - 1] < quality) {
       	        			read[j + first_index - 1] = next_query[j];
					qualities[j + first_index - 1] = quality;
				}
			}
		}
	}
	else {
		for (int j = 1; j < strlen(next_query) + 1; j++) {
			if ((first_index - j) >= 0) {
				if (qualities[first_index - j] < quality) {
      	 				if (next_query[first_index - j] == 65) // 'A'
					...
					qualities[first_index - j] = quality;
				}
			}
		}
	}
...
/*
 * findQuality takes the header line parsed and converts the query's quality
 * value from string to double and returns it. 
*/
double findQuality(char *header) {
	int j = 0;
	char string_val[100] = "";

	j = findChar(j, header, '!') + 1;
	j = findChar(j, header, '!') + 1;

	for (int i = 0; i < strlen(header); i++)
		string_val[i] = header[i + j];
	double quality = atof(string_val);
